...
const liveUrlNode = document.getElementById('live-url');
const copyButton = document.getElementById('copy-url');

if (liveUrlNode) {
  const absoluteUrl = `${window.location.origin}${window.location.pathname.replace(/[^/]+$/, '')}live.html`;
  liveUrlNode.textContent = absoluteUrl;
}

if (copyButton && liveUrlNode) {
  copyButton.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(liveUrlNode.textContent);
